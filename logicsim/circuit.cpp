    ...
    while (getline(inFile, line)) {
        ...
        if (line == "GATES") {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (s_type == "AND2") {
                    ...
                    getline(ss, s_in2, ',');
                    getline(ss, s_output, ',');
                    m_gates.push_back(new And2Gate(m_wires[std::stoi(s_in1)],
                                                   m_wires[std::stoi(s_in2)],
                                                   m_wires[std::stoi(s_output)]));
                }
                else if (s_type == "OR2") {
                    std::string s_in1, s_in2, s_output;
                    getline(ss, s_in1, ',');
                    getline(ss, s_in2, ',');
                    getline(ss, s_output, ',');
                    m_gates.push_back(new Or2Gate(m_wires[std::stoi(s_in1)],
                                                  m_wires[std::stoi(s_in2)],
                                                  m_wires[std::stoi(s_output)]));
                }
                else if (s_type == "NOT") {
                    ...
                    getline(ss, s_in, ',');
                    getline(ss, s_output, ',');
                    m_gates.push_back(new NotGate(m_wires[std::stoi(s_in)],
                                                  m_wires[std::stoi(s_output)]));
                }
            }
        }
        if (line == "INJECT") {
            ...
            for (int i = 0; i < n; i++) {
                ...
                getline(ss, s_wire, ',');
                getline(ss, s_state, ',');
                Event* e = new Event { static_cast<uint64_t>(std::stoi(s_time)),
                                        m_wires[std::stoi(s_wire)],
                                        s_state[0] };
                m_pq.push(e);
            }
        }
